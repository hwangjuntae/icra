            ...
            if self.yolo_model is not None:
                ...
                if len(yolo_results) > 0 and yolo_results[0].boxes is not None:
                    ...
                    for detection in yolo_results[0].boxes:
                        ...
                        # 객체 영역 추출 (다운샘플된 이미지에서)
                        obj_x1 = x1 / scale_info['scale_x'] if scale_info else x1
                        obj_y1 = y1 / scale_info['scale_y'] if scale_info else y1
                        obj_x2 = x2 / scale_info['scale_x'] if scale_info else x2
                        obj_y2 = y2 / scale_info['scale_y'] if scale_info else y2
                        
                        obj_image = image[int(obj_y1):int(obj_y2), int(obj_x1):int(obj_x2)]
                        if obj_image.size > 0:
                            # 배치 처리를 위해 객체 정보 저장
                            obj_info = {
                                'image': obj_image,
                                'class_name': class_name,
                                'bbox_idx': detection_count
                            }
                            self.blip_queue.append(obj_info)
                            
                        # 임시로 기본 설명 사용 (배치 처리 후 업데이트됨)
                        obj_description = f"{class_name} 객체"
